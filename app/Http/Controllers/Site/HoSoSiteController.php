...
class HoSoSiteController extends Controller
{
    ...
    {
        $category_search = Category::with('informations')->where('search', 1)->where('status', 1)->where('type', '!=', 2)->orderBy('order', 'ASC')->get();
        $category_noibat = Category::with('informations')->where('noi_bat', 1)->where('status', 1)->where('type', '!=', 2)->orderBy('order', 'ASC')->get();
        $hoso_list = HoSoXinViec::with('informations', 'provinces', 'users')->where('status', 1)->orderBy('stt', 'ASC')->get();
    ...
    public function searchInformation(Request $request)
    {
        ...
        if ($request->information != null && is_array($request->information)) {
            $hoso_list = HoSoXinViec::with('informations', 'provinces', 'users')->where('hosoxinviec.status', 1)->distinct()->select('hosoxinviec.*')
        ...
        return view('site.ajax.ajax_hoSoXinViec',compact('hoso_list','category_noibat'));
    }
