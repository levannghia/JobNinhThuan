...
class RecruitmentController extends Controller
{
    ...
    {
        $recruiment_list = '';
        $category_search = Category::with('informations')->where('search', 1)->where('status', 1)->where('type', '!=', 1)->orderBy('order', 'ASC')->get();
        $category_noibat = Category::with('informations')->where('noi_bat', 1)->where('status', 1)->where('type', '!=', 1)->orderBy('order', 'ASC')->get();
        if(isset($request->keyword) || isset($request->information_id) || isset($request->province_id)){  
            $recruiment_list = Recruitment::select('recruitments.*')->distinct()
                ->join('recruitment_information', 'recruitments.id', '=', 'recruitment_information.recruitment_id')
                ->join('recruitment_province', 'recruitments.id', '=', 'recruitment_province.recruitment_id');
                // ->where('recruitment_information.information_id', $request->information_id)
                // ->where('recruitment_province.province_matp', $request->province_id)
                // ->get();
            if(!empty($request->information_id)){
                $recruiment_list->where('recruitment_information.information_id', $request->information_id);
            }
            if(!empty($request->province_id)){
                $recruiment_list->where('recruitment_province.province_matp', $request->province_id);
            }
            if(!empty($request->keyword)){
                $recruiment_list->where("vi_tri", "like", '%' . $request->keyword . '%');
            }
            $recruiment_list = $recruiment_list->get();

        }else{
            $recruiment_list = Recruitment::with('informations','provinces','Employers')->where('status', 1)->orderBy('stt', 'ASC')->get();
    ...
    public function searchInformation(Request $request)
    {
        ...
        if ($request->information != null && is_array($request->information)) {
            $recruiment_list = Recruitment::with('informations','provinces','Employers')->distinct()->select('recruitments.*')
                ->join('recruitment_information', 'recruitments.id', '=', 'recruitment_information.recruitment_id')
        ...
        // return $data;
        return view('site.ajax.ajax_recruiment',compact('recruiment_list','category_noibat'));
    }
